    ...
    system: {
            ...
            loadPadSim() {
                ...
                input.addEventListener('input', function () {
                    ...
                    reader.addEventListener('load', () => {
                        ...
                        mknpad.system.pad.sim2mkn(reader.result);
                    });
            ...
            internal: {
                ...
                subConvert(buf, blk, name) {
                    ...
                    for (let n = 0; n < count; n++) {
                        ...
                        switch (type) {
                            case 0x01: /* normal */
                                mknpad.system.block.internal.createNormal(emps.item(emps.length - 1), cmd);
                                break;
                            case 0x02: /* if */
                                {
                                    mknpad.system.block.internal.createIf(emps.item(emps.length - 1), cmd);
                            ...
                            case 0x03: /* while */
                                {
                                    mknpad.system.block.internal.createWhile(emps.item(emps.length - 1), cmd);
                                    let whls = blk.getElementsByClassName(mknpad.const.blocks.whlblk);
                                    mknpad.system.pad.internal.subConvert(buf, whls.item(whls.length - 1).nextElementSibling, `WHILE (${cmd})`);
                                }
                                break;
                            case 0x04: /* until */
                                {
                                    mknpad.system.block.internal.createUntil(emps.item(emps.length - 1), cmd);
                                    let dows = blk.getElementsByClassName(mknpad.const.blocks.dowblk);
                                    mknpad.system.pad.internal.subConvert(buf, dows.item(dows.length - 1).nextElementSibling, `UNTIL (${cmd})`);
                                }
                                break;
                            case 0x05: /* output */
                                mknpad.system.block.internal.createNormal(emps.item(emps.length - 1), `print(${cmd})`);
                                break;
                            case 0x06: /* input */
                                mknpad.system.block.internal.createNormal(emps.item(emps.length - 1), `scan(${cmd})`);
                                break;
                            default:
        ...
        block: {
            internal: {
                ...
                createWhile(ClickedEmptyBlock, CBCommand) {
                ...
                createUntil(ClickedEmptyBlock, CBCommand) {
                    let WB = document.createElement('div');
                ...
                createIf(ClickedEmptyBlock, CBCommand) {
                    let WB = document.createElement('div');
                ...
                createComment(ClickedEmptyBlock, CBCommand) {
                ...
                createFunction(ClickedEmptyBlock, CBCommand) {
                    let CB = document.createElement('div');
                    let tmp;
                ...
                createNormal(ClickedEmptyBlock, CBCommand) {
                    let CB = document.createElement('div');
